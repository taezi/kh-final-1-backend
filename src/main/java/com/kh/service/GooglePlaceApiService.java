    ...
    import com.kh.dto.CafeDto; // CafeDto 클래스를 import 합니다.
    ...
    public class GooglePlaceApiService {
        ...
        // 로거 설정
        private static final Logger logger = LoggerFactory.getLogger(GooglePlaceApiService.class);
        ...
        public GooglePlaceApiService(WebClient.Builder webClientBuilder, ObjectMapper objectMapper) {
         ...
         * @param cafeBranch 카페 지점명
         * @return API로부터 보완된 CafeDto 객체. API 호출 실패 시 Mono.empty()를 반환합니다.
         */
        public Mono<CafeDto> getCafeDetails(CafeDto cafeDtoFromDb, String cafeName, String cafeBranch) {

            // 데이터베이스에 이미 카페 정보가 있고 이미지 주소도 있다면, 그대로 반환
            if (cafeDtoFromDb != null && cafeDtoFromDb.getCafeImgAddress() != null && !cafeDtoFromDb.getCafeImgAddress().isEmpty()) {
                logger.info("Database already contains cafe details with an image. Returning existing data.");
                return Mono.just(cafeDtoFromDb);
            }

            // 단일 검색: 카페 이름과 지점명을 합쳐 한 번에 검색을 시도합니다.
            String query = cafeName;
            if (cafeBranch != null && !cafeBranch.trim().isEmpty()) {
                query += " " + cafeBranch;
            }
         ...
         * @param cafeDtoFromDb 데이터베이스에서 가져온 기존 CafeDto 객체
         * @return API로부터 보완된 CafeDto 객체. API 호출 실패 시 Mono.empty()를 반환합니다.
         */
            ...
            return webClient.get()
                    ...
                    .flatMap(json -> {
                        try {
                            ...
                            CafeDto finalCafeDto = (cafeDtoFromDb != null) ? cafeDtoFromDb : new CafeDto();

                            // API 응답에서 필요한 데이터를 추출하여 DTO에 설정
                            if (result.has("formatted_address")) {
                                finalCafeDto.setCafeAddress(result.path("formatted_address").asText(""));
                            }
                            if (result.has("website")) {
                                finalCafeDto.setCafeWebsite(result.path("website").asText(""));
                            }
                            ...
                            if (result.has("opening_hours") && result.path("opening_hours").has("weekday_text")) {
                                ...
                                if (openingHours.isArray() && openingHours.size() > 0) {
                                    finalCafeDto.setCafeOpen(openingHours.path(0).asText(""));
                                }
                            }
                            ...
                            if (photos.isArray() && photos.size() > 0) {
                                ...
                                finalCafeDto.setCafeImgAddress(photoUrl);
                            } else {
                                logger.info("No photos found for place_id: {}", placeId);
                            }
                            return Mono.just(finalCafeDto);
                        } catch (Exception e) {
